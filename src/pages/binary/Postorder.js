...
const Postorder = ({ sidebarWidth }) => {
  ...
  const PostorderTraversalWithAnimation = async (node) => {
    if (!node) return;
  
    if (node.children && node.children.length > 0) {
      for (const child of node.children) {
        await PostorderTraversalWithAnimation(child);
      }
    }
  ...
  const handlePostorderNodeWithAnimation = async () => {
    if (!treeData) return;
    d3.selectAll("circle").attr("fill", "#90caf9");
    setVisitedNodes([]);
    setIsRunning(true);
    await PostorderTraversalWithAnimation(treeData);
    setIsRunning(false);
  };
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "16px", padding: "16px", marginLeft: sidebarWidth }}>
      ...
      <Typography variant="h6" sx={{ mt: 4, color: "#1e88e5", fontWeight: "bold", border: "1px solid #90caf9", padding: "12px", borderRadius: "8px", backgroundColor: "#e3f2fd", width: "100%", textAlign: "center" }}>
        Visited Nodes (Postorder): {visitedNodes.join(" → ")}
      </Typography>
            ...
            <Button
              ...
              onClick={() => {
                if (isPaused) {
                  ...
                  handlePostorderNodeWithAnimation();
                }
              }}
            >
