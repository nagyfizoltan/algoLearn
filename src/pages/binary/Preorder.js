...
const Preorder = ({ sidebarWidth }) => {
  ...
  const preorderTraversalWithAnimation = async (node) => {
    if (!node) return;
    ...
    for (const child of node.children || []) {
      await preorderTraversalWithAnimation(child);
    }

    highlightCurrentNode(node.name, "#90caf9");
  };

  const handlePreorderNodeWithAnimation = async () => {
    if (!treeData) return;
    d3.selectAll("circle").attr("fill", "#90caf9");
    setVisitedNodes([]);
    setIsRunning(true);
    await preorderTraversalWithAnimation(treeData);
    setIsRunning(false);
  };
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "16px", padding: "16px", marginLeft: sidebarWidth }}>
      ...
      <Typography variant="h6" sx={{ mt: 4, color: "#1e88e5", fontWeight: "bold", border: "1px solid #90caf9", padding: "12px", borderRadius: "8px", backgroundColor: "#e3f2fd", width: "100%", textAlign: "center" }}>
        Visited Nodes (Preorder): {visitedNodes.join(" → ")}
      </Typography>
            ...
            <Button
              ...
              onClick={() => {
                if (isPaused) {
                  ...
                  handlePreorderNodeWithAnimation();
                }
              }}
            >
